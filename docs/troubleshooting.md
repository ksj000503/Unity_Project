...
## 4. mcp-unity는 동시에 하나의 클라이언트 연결만 처리한다

**상황**

Task 4(BattleMath) 구현을 서브에이전트(subagent)에게 위임했는데, 서브에이전트가 호출하는 `mcp-unity` 도구(`get_console_logs`, `execute_menu_item` 등)가 전부 응답 없이 걸려 있다가 타임아웃났다(처음엔 60초 큐 타임아웃, 재시도 후엔 1800초 유휴 타임아웃까지). Unity 에디터가 켜져 있고 Safe 모드도 벗어난 상태였는데도 재현됐다.

**원인 분석**

같은 시점에 컨트롤러(메인) 세션에서 직접 `get_console_logs`를 호출하면 즉시 정상 응답이 왔다 — 즉 Unity 쪽 브리지 자체는 살아있고 정상 작동 중이었다. 반면 서브에이전트는 별도의 프로세스로 자기만의 `mcp-unity` MCP 서버 인스턴스를 새로 띄우고, 그 인스턴스가 Unity의 웹소켓 브리지(포트 8090)에 다시 연결을 시도한다. 이때 Unity 쪽이 이미 컨트롤러 세션의 연결을 붙잡고 있어서, 서브에이전트의 연결 요청은 큐에 쌓이기만 하고 응답을 받지 못했다. 즉 `mcp-unity`의 Unity 측 웹소켓 서버는 사실상 동시에 클라이언트 하나만 제대로 서비스하는 구조로 보인다.

**해결**

서브에이전트에게 위임한 Task 4 작업을 중단(`TaskStop`)시키고, 이미 살아있는 연결을 가진 컨트롤러 세션이 직접 나머지 단계(구현 파일 작성, `Assets/Refresh`, 테스트 실행, 스테이징)를 수행해 완료했다.

**배운 점**

mcp-unity처럼 외부 애플리케이션(Unity 에디터)과 단일 세션으로 연결되는 MCP 서버는, 여러 세션/서브에이전트가 동시에 쓰는 걸 전제로 설계되지 않았을 수 있다. subagent-driven-development 방식으로 여러 에이전트를 병렬 디스패치하더라도, Unity 에디터를 직접 조작해야 하는 단계(에셋 리프레시, 테스트 실행, 씬 조작 등)는 mcp-unity 연결을 이미 쥐고 있는 세션(이 경우 컨트롤러)이 직접 수행하는 걸로 방침을 바꿨다. 도구의 동시성 제약은 문서화되어 있지 않은 경우가 많아서, 이런 건 실제로 병렬 호출을 시도해봐야 드러난다.
